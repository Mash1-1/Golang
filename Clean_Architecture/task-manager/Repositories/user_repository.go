package repositories

import (
	"context"

	"task_manager_ca/Domain"

	"go.mongodb.org/mongo-driver/bson"
	"go.mongodb.org/mongo-driver/mongo"
	"go.mongodb.org/mongo-driver/mongo/options"
)

type UserRepo struct {
	UserDatabase *mongo.Collection
}

func NewUserRepo(db *mongo.Collection) UserRepo{
	return UserRepo{
		UserDatabase: db,
	}
}

func InitializeUserDB() (*mongo.Collection) {
	// Initialize client
	clientOptions := options.Client().ApplyURI("mongodb://localhost:27017")
	client, err := mongo.Connect(context.TODO(), clientOptions)

	// Handle error 
	if err != nil {
		return &mongo.Collection{}
	}
	// Check if connection is established
	err = client.Ping(context.TODO(), nil)
	if err != nil {
		return &mongo.Collection{}
	}

	collection := client.Database("user_db").Collection("users")
	// Clear previous data instances 
	collection.DeleteMany(context.TODO(), bson.D{{}})

	// Assign new database into a global variable
 	return collection
}

func (ur *UserRepo) FindUserRepository(user_name string) bool {
	filter := bson.D{{Key: "username", Value: user_name}}
	var tmp_user Domain.User
	err := ur.UserDatabase.FindOne(context.TODO(), filter).Decode(&tmp_user)
	if err == nil { // Username already in the database 
		return true
	}
	return false 
}

func (ur *UserRepo) Create(user Domain.User) error {
	// Add new user into the database
	_, err := ur.UserDatabase.InsertOne(context.TODO(), user)
	return err
}

func (ur *UserRepo)Login(user Domain.User) (Domain.User, error){
	var existingUser Domain.User 
	// User a filter that has the email we are looking for 
	filter := bson.D{{Key: "username", Value: user.Username}}

	// Search for the user in the database
	err := ur.UserDatabase.FindOne(context.TODO(), filter).Decode(&existingUser)
	// Handle error when the user is not found 
	if err != nil {
		return Domain.User{}, err
	}
	return existingUser, nil 
}